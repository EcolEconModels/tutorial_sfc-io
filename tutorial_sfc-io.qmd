...
These setup chunks keep only minimal imports/options/config (no model logic).


This chunk checks required packages and plotting defaults.
...
This chunk defines the core configuration used in Steps 1-4.

```{r setup_core_config}
# Environment overrides: SFC_IO_COUNTRY, SFC_IO_YEAR, SFC_IO_SCOPE, SFC_IO_TABLE_TYPE
core_config <- list(
  country = Sys.getenv("SFC_IO_COUNTRY", "AT"), year = as.integer(Sys.getenv("SFC_IO_YEAR", "2020")),
  scope = Sys.getenv("SFC_IO_SCOPE", "TOTAL"), table_type = Sys.getenv("SFC_IO_TABLE_TYPE", "product_by_product"),
  freq = "A", iot_unit = "MIO_EUR", wealth_unit = "MIO_EUR",
  wealth_co_nco = "NCO", wealth_na_item = "BF90", wealth_finpos = "LIAB",
  emissions_airpol = "CO2", emissions_unit = "THS_T", cache_dir = "data", enforce_co2_consistency = FALSE
)
```

This chunk defines the emissions configuration used in Step 5.

```{r setup_emissions_config}
# Environment overrides: SFC_EMIS_COUNTRY, SFC_EMIS_YEAR, SFC_EMIS_SCOPE, SFC_EMIS_TABLE_TYPE, SFC_EMIS_AIRPOL
emissions_config <- list(
  country = Sys.getenv("SFC_EMIS_COUNTRY", "BE"), year = as.integer(Sys.getenv("SFC_EMIS_YEAR", "2020")),
  scope = Sys.getenv("SFC_EMIS_SCOPE", "TOTAL"), table_type = Sys.getenv("SFC_EMIS_TABLE_TYPE", "industry_by_industry"),
  freq = "A", iot_unit = "MIO_EUR", wealth_unit = "MIO_EUR",
  wealth_co_nco = "NCO", wealth_na_item = "BF90", wealth_finpos = "LIAB",
  emissions_airpol = Sys.getenv("SFC_EMIS_AIRPOL", "CO2"), emissions_unit = "THS_T", cache_dir = "data", enforce_co2_consistency = TRUE
)
```
...
This chunk defines `sim_steady_state()` for step1.

```{r step1_functions_sim_steady_state}
...
This chunk defines `simulate_sim_lag()` for step1.

```{r step1_functions_simulate_sim_lag}
simulate_sim_lag <- function(T = 20, G = 100, theta = 0.2, alpha1 = 0.6, alpha2 = 0.2, Y0 = 80, H0 = 300) {
  ...
  YD_prev <- (1 - theta) * Y0; H_prev <- H0
  for (tt in seq_len(T)) {
    C_t <- alpha1 * YD_prev + alpha2 * H_prev; Y_t <- C_t + G
    T_t <- theta * Y_t; YD_t <- Y_t - T_t; H_t <- H_prev + YD_t - C_t
    out[tt, c("Y", "T_tax", "YD", "C", "H")] <- c(Y_t, T_t, YD_t, C_t, H_t)
    YD_prev <- YD_t; H_prev <- H_t
  }
  out
}
```

This chunk defines `simulate_sim_current_fp()` for step1.

```{r step1_functions_simulate_sim_current_fp, echo: false}
...
This walkthrough chunk summarizes `simulate_sim_current_fp()` logic in small steps.


This chunk runs `step1_walk_simulate_sim_current_fp`.

```{r step1_walk_simulate_sim_current_fp}
# Outline for simulate_sim_current_fp()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
invisible(NULL)
```


This chunk runs step1 and produces the step outputs.

```{r step1_run_setup}
T_h <- 20; pars <- list(G = 100, theta = 0.2, alpha1 = 0.6, alpha2 = 0.2)
ss <- sim_steady_state(pars$G, pars$theta, pars$alpha1, pars$alpha2)
Y0_low <- 0.7 * ss$Y_star; Y0_high <- 1.3 * ss$Y_star
...
This chunk runs both SIM variants and assembles comparable GDP paths.

```{r step1_run_paths}
lag_low <- simulate_sim_lag(T = T_h, G = pars$G, theta = pars$theta, alpha1 = pars$alpha1, alpha2 = pars$alpha2, Y0 = Y0_low, H0 = H0_low)
lag_high <- simulate_sim_lag(T = T_h, G = pars$G, theta = pars$theta, alpha1 = pars$alpha1, alpha2 = pars$alpha2, Y0 = Y0_high, H0 = H0_high)
cur_low <- simulate_sim_current_fp(T = T_h, G = pars$G, theta = pars$theta, alpha1 = pars$alpha1, alpha2 = pars$alpha2, Y0 = Y0_low, H0 = H0_low)
cur_high <- simulate_sim_current_fp(T = T_h, G = pars$G, theta = pars$theta, alpha1 = pars$alpha1, alpha2 = pars$alpha2, Y0 = Y0_high, H0 = H0_high)
step1_paths <- rbind(
...
This chunk plots results for step1.
...
This chunk loads closure utilities used by the Step 2 transition solver.

```{r step2_closure_setup}
source("closure_utils.R")
```

This chunk defines `validate_tutorial_config()` for step2.

```{r step2_functions_validate_tutorial_config}
...
This chunk defines `as_numeric_clean()` for step2.

```{r step2_functions_as_numeric_clean}
...
This chunk defines `safe_div()` for step2.

```{r step2_functions_safe_div}
...
This chunk defines `ensure_cache_dir()` for step2.

```{r step2_functions_ensure_cache_dir}
...
This chunk defines `sanitize_for_filename()` for step2.

```{r step2_functions_sanitize_for_filename}
...
This chunk defines `cache_file_for_query()` for step2.

```{r step2_functions_cache_file_for_query}
...
This chunk defines `fetch_eurostat_json()` for step2.

This chunk builds URL query parts for Eurostat API requests.

```{r step2_functions_build_query_parts}
build_query_parts <- function(params) {
  q <- character(0)
  for (nm in names(params)) {
    vv <- params[[nm]]; if (length(vv) == 0) next
...
This chunk fetches Eurostat JSON (cache-first, download fallback).

```{r step2_functions_fetch_eurostat_json}
fetch_eurostat_json <- function(dataset_id, params, cache_dir = "data") {
  cache_dir <- ensure_cache_dir(cache_dir); cache_file <- cache_file_for_query(dataset_id, params, cache_dir)
  ...
  if (!ok || !file.exists(cache_file)) stop("Eurostat download failed for ", dataset_id, ". URL: ", url, call. = FALSE)
  jsonlite::read_json(cache_file, simplifyVector = FALSE)
}
```

This chunk defines `ordered_categories()` for step2.

```{r step2_functions_ordered_categories}
...
This chunk defines `decode_jsonstat_index()` for step2.

```{r step2_functions_decode_jsonstat_index}
...
This chunk defines `extract_matrix_from_json()` for step2.

```{r step2_functions_extract_matrix_from_json}
extract_matrix_from_json <- function(js, row_dim, col_dim) {
  ids <- unlist(js[["id"]]); sizes <- as.integer(unlist(js[["size"]]))
  row_i <- match(row_dim, ids); col_i <- match(col_dim, ids)
  if (is.na(row_i) || is.na(col_i)) stop("Could not find row/col dimensions in dataset.", call. = FALSE)
  row_cat <- ordered_categories(js[["dimension"]][[row_dim]]); col_cat <- ordered_categories(js[["dimension"]][[col_dim]])
  vals <- unlist(js[["value"]]); if (length(vals) == 0) stop("No observations in returned dataset for requested filters.", call. = FALSE)
  M <- matrix(NA_real_, nrow = nrow(row_cat), ncol = nrow(col_cat)); flat_idx <- as.integer(names(vals))
  for (j in seq_along(flat_idx)) {
    pos <- decode_jsonstat_index(flat_idx[j], sizes); rr <- pos[row_i] + 1L; cc <- pos[col_i] + 1L; M[rr, cc] <- as.numeric(vals[[j]])
  }
  list(M = M, row_cat = row_cat, col_cat = col_cat, ids = ids, sizes = sizes)
}
```

This chunk defines `extract_values_by_dim()` for step2.

```{r step2_functions_extract_values_by_dim}
extract_values_by_dim <- function(js, dim_name) {
  ids <- unlist(js[["id"]]); sizes <- as.integer(unlist(js[["size"]])); dim_i <- match(dim_name, ids)
  if (is.na(dim_i)) stop("Dimension not found: ", dim_name, call. = FALSE)
  cat_tbl <- ordered_categories(js[["dimension"]][[dim_name]])
  vals <- unlist(js[["value"]]); if (length(vals) == 0) return(setNames(numeric(0), character(0)))
  out <- rep(NA_real_, nrow(cat_tbl)); names(out) <- cat_tbl$code; flat_idx <- as.integer(names(vals))
  for (j in seq_along(flat_idx)) {
    pos <- decode_jsonstat_index(flat_idx[j], sizes); key <- cat_tbl$code[pos[dim_i] + 1L]; out[key] <- as.numeric(vals[[j]])
  }
  out
}
```
...
This chunk defines `iot_schema()` for step2.

```{r step2_functions_iot_schema}
...
This chunk defines `sum_matching_cols()` for step2.

```{r step2_functions_sum_matching_cols}
...
This chunk defines `select_sector_codes()` for step2.

```{r step2_functions_select_sector_codes}
select_sector_codes <- function(row_codes, col_codes, x_by_row) {
  exclude_codes <- c("TOTAL", "TU", "TFU", "TS_BP", "IMP", "B1G", "B2A3N", "B2A3G", "B3G", "P1", "P2_ADJ", "D21X31", "P7", "P7_B0", "P7_D0", "P7_U2", "P7_U3")
  candidate <- setdiff(intersect(row_codes, col_codes), exclude_codes)
  valid_x <- names(x_by_row)[is.finite(x_by_row) & x_by_row > 1e-9]
  candidate <- intersect(candidate, valid_x)
  if (length(candidate) < 20) stop("Could not identify sector set from IOT matrix.", call. = FALSE)
  candidate
}
```


This chunk prepares row/column metadata and sector selection from the SIOT matrix.

```{r step2_functions_prepare_iot_selection}
prepare_iot_selection <- function(mat) {
  row_codes <- mat$row_cat$code; row_labels <- mat$row_cat$label
  col_codes <- mat$col_cat$code; col_labels <- mat$col_cat$label
  tu_col <- match("TU", col_codes); if (is.na(tu_col)) tu_col <- match("TOTAL", col_codes)
  if (is.na(tu_col)) stop("Could not find TU/TOTAL use column in IOT data.", call. = FALSE)
  x_by_row <- mat$M[, tu_col]; names(x_by_row) <- row_codes
  sector_codes <- select_sector_codes(row_codes, col_codes, x_by_row)
  r_idx <- match(sector_codes, row_codes); c_idx <- match(sector_codes, col_codes)
  list(
    row_codes = row_codes, row_labels = row_labels,
    col_codes = col_codes, col_labels = col_labels,
    x_by_row = x_by_row, sector_codes = sector_codes,
    r_idx = r_idx, c_idx = c_idx
  )
}
```

This chunk computes `Z`, `A`, and `L` for the selected sectors.

```{r step2_functions_compute_iot_technology}
compute_iot_technology <- function(mat, sel) {
  Z <- mat$M[sel$r_idx, sel$c_idx, drop = FALSE]
  x0 <- as.numeric(sel$x_by_row[sel$sector_codes]); names(x0) <- sel$sector_codes
  x_safe <- x0; x_safe[!is.finite(x_safe) | x_safe <= 0] <- 1e-9
  A0 <- sweep(Z, 2, x_safe, "/"); A0[!is.finite(A0)] <- 0
  L0 <- tryCatch(
    solve(diag(length(sel$sector_codes)) - A0),
    error = function(e) solve(diag(length(sel$sector_codes)) - A0 + diag(1e-8, length(sel$sector_codes)))
  )
  list(Z = Z, x0 = x0, A0 = A0, L0 = L0)
}
```

This chunk extracts final-demand shares, import leakage, and value-added coefficients.

```{r step2_functions_extract_iot_fd}
extract_iot_fd <- function(mat, sel, tech) {
  Msel <- mat$M[sel$r_idx, , drop = FALSE]
  C_i0 <- sum_matching_cols(Msel, sel$col_codes, sel$col_labels, c("^P3_S14$", "households"))
  C_i0 <- C_i0 + sum_matching_cols(Msel, sel$col_codes, sel$col_labels, c("^P3_S15$", "NPISH"))
  G_i0 <- sum_matching_cols(Msel, sel$col_codes, sel$col_labels, c("^P3_S13$", "government"))
  I_i0 <- sum_matching_cols(Msel, sel$col_codes, sel$col_labels, c("^P51G$", "^P52$", "^P53$", "^P5$", "gross capital"))
  EX_i0 <- sum_matching_cols(Msel, sel$col_codes, sel$col_labels, c("^P6$", "^P6_", "exports"))
  imp_row <- which(sel$row_codes %in% c("P7", "IMP")); if (length(imp_row) == 0) imp_row <- grep("imports", sel$row_labels, ignore.case = TRUE)
  imports_i <- if (length(imp_row) > 0) as.numeric(mat$M[imp_row[1], sel$c_idx, drop = TRUE]) else rep(0, length(sel$sector_codes))
  m_i <- pmin(pmax(safe_div(imports_i, tech$x0), 0), 0.95)
  va_row <- which(sel$row_codes == "B1G")
  va_coeff <- if (length(va_row) > 0) safe_div(as.numeric(mat$M[va_row[1], sel$c_idx, drop = TRUE]), tech$x0) else pmax(1 - colSums(tech$A0, na.rm = TRUE), 0)
  va_coeff[!is.finite(va_coeff) | va_coeff < 0] <- 0
  beta_C <- safe_div(C_i0, rep(sum(C_i0), length(C_i0))); beta_G <- safe_div(G_i0, rep(sum(G_i0), length(G_i0)))
  if (sum(beta_C) > 0) beta_C <- beta_C / sum(beta_C); if (sum(beta_G) > 0) beta_G <- beta_G / sum(beta_G)
  list(C_i0 = C_i0, G_i0 = G_i0, I_i0 = I_i0, EX_i0 = EX_i0, m_i = m_i, va_coeff = va_coeff, beta_C = beta_C, beta_G = beta_G)
}
```

This chunk defines `download_or_load_iot()` for step2.

```{r step2_functions_download_or_load_iot, echo: false}
download_or_load_iot <- function(cfg) {
  ...
  mat <- tryCatch(
    ...
    error = function(e) stop(
      ...
      ", year=", cfg$year, ", scope=", cfg$scope, ", table_type=", cfg$table_type, ". ", e$message,
      call. = FALSE
    )
  )
  sel <- prepare_iot_selection(mat)
  tech <- compute_iot_technology(mat, sel)
  fd <- extract_iot_fd(mat, sel, tech)
  list(
    base_year = cfg$year, country = cfg$country, scope = cfg$scope, table_type = cfg$table_type, dataset_id = schema$dataset_id,
    n = length(sel$sector_codes), sector_codes = sel$sector_codes, sector_labels = sel$row_labels[sel$r_idx],
    Z0 = tech$Z, A0 = tech$A0, L0 = tech$L0, x0 = tech$x0,
    C_i0 = fd$C_i0, G_i0 = fd$G_i0, I_i0 = fd$I_i0, EX_i0 = fd$EX_i0,
    F0 = fd$C_i0 + fd$G_i0 + fd$I_i0 + fd$EX_i0, m_i = fd$m_i, va_coeff = fd$va_coeff,
    Y0 = sum(fd$va_coeff * tech$x0), G0 = sum(fd$G_i0), beta_C = fd$beta_C, beta_G = fd$beta_G
  )
}
```

This walkthrough chunk summarizes `download_or_load_iot()` logic in small steps.


This chunk runs `step2_walk_download_or_load_iot`.

```{r step2_walk_download_or_load_iot}
# Outline for download_or_load_iot()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk defines `find_energy_indices()` for step2.

```{r step2_functions_find_energy_indices}
...
This chunk defines `simulate_iot_exogenous_transition()` for step2.

```{r step2_functions_simulate_iot_exogenous_transition, echo: false}
...
This walkthrough chunk summarizes `simulate_iot_exogenous_transition()` logic in small steps.


This chunk runs `step2_walk_simulate_iot_exogenous_transition`.

```{r step2_walk_simulate_iot_exogenous_transition}
# Outline for simulate_iot_exogenous_transition()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk runs step2 and produces the step outputs.
...
This chunk plots results for step2.
...
This chunk defines `load_wealth_init()` for step3.

```{r step3_functions_load_wealth_init, echo: false}
...
This walkthrough chunk summarizes `load_wealth_init()` logic in small steps.


This chunk runs `step3_walk_load_wealth_init`.

```{r step3_walk_load_wealth_init}
# Outline for load_wealth_init()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk defines `calibrate_sim_iot()` for step3.

```{r step3_functions_calibrate_sim_iot}
calibrate_sim_iot <- function(iot, wealth_init, tau_y = NULL) {
  ...
  list(
    base_year = iot$base_year, n = iot$n, sector_codes = iot$sector_codes, sector_labels = iot$sector_labels,
    L = iot$L0, va_coeff = iot$va_coeff, beta_C = iot$beta_C, beta_G = iot$beta_G, I_i0 = iot$I_i0,
    G0 = iot$G0, tau_y = tau_guess, V0 = wealth_init$V0, B0 = wealth_init$B0,
    idx = find_energy_indices(iot$sector_codes, iot$sector_labels)
  )
}
```

This chunk defines `simulate_sim_iot_endogenous()` for step3.

```{r step3_functions_simulate_sim_iot_endogenous, echo: false}
...
This walkthrough chunk summarizes `simulate_sim_iot_endogenous()` logic in small steps.


This chunk runs `step3_walk_simulate_sim_iot_endogenous`.

```{r step3_walk_simulate_sim_iot_endogenous}
# Outline for simulate_sim_iot_endogenous()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk runs step3 and produces the step outputs.
...
This chunk plots results for step3.
...
This chunk defines `calibrate_sim_iot_row()` for step4.

```{r step4_functions_calibrate_sim_iot_row}
calibrate_sim_iot_row <- function(iot, wealth_init, tau_y = NULL) {
  tau_guess <- if (is.null(tau_y)) min(max(iot$G0 / pmax(iot$Y0, 1), 0.1), 0.5) else tau_y
  list(
    base_year = iot$base_year, n = iot$n, sector_codes = iot$sector_codes, sector_labels = iot$sector_labels,
    L = iot$L0, va_coeff = iot$va_coeff, beta_C = iot$beta_C, beta_G = iot$beta_G, I_i0 = iot$I_i0, EX_i0 = iot$EX_i0,
    m_i = iot$m_i, G0 = iot$G0, tau_y = tau_guess, V0 = wealth_init$V0, B0 = wealth_init$B0,
    idx = find_energy_indices(iot$sector_codes, iot$sector_labels)
  )
}
```

This chunk defines `simulate_sim_iot_row_lite()` for step4.

```{r step4_functions_simulate_sim_iot_row_lite, echo: false}
...
This walkthrough chunk summarizes `simulate_sim_iot_row_lite()` logic in small steps.


This chunk runs `step4_walk_simulate_sim_iot_row_lite`.

```{r step4_walk_simulate_sim_iot_row_lite}
# Outline for simulate_sim_iot_row_lite()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk runs step4 and produces the step outputs.
...
This chunk plots results for step4.
...
This chunk normalizes NACE sector codes for emissions matching in step5.

```{r step5_functions_normalize_nace_code}
...
This chunk extracts available AEA sector codes for the selected emissions configuration.

```{r step5_functions_get_aea_codes}
get_aea_codes <- function(cfg) {
  ...
  unique(normalize_nace_code(nz_codes))
}
```

This chunk subsets IOT sectors to those with available AEA coverage when strict consistency is enabled.

```{r step5_functions_align_iot_to_aea}
align_iot_to_aea <- function(iot, cfg) {
  aea_norm <- get_aea_codes(cfg); sec_norm <- normalize_nace_code(iot$sector_codes)
  keep <- sec_norm %in% aea_norm
  if (sum(keep) < 20) stop("Too few sectors overlap between IOT and AEA for selected emissions configuration.", call. = FALSE)
  idx <- which(keep)
  iot$sector_codes <- iot$sector_codes[idx]; iot$sector_labels <- iot$sector_labels[idx]; iot$n <- length(idx)
  iot$Z0 <- iot$Z0[idx, idx, drop = FALSE]; iot$A0 <- iot$A0[idx, idx, drop = FALSE]
  ...
  iot$G0 <- sum(iot$G_i0); iot$Y0 <- sum(iot$va_coeff * iot$x0)
...
This chunk defines `load_aea_emissions()` for step5.

```{r step5_functions_load_aea_emissions, echo: false}
...
This walkthrough chunk summarizes `load_aea_emissions()` logic in small steps.


This chunk runs `step5_walk_load_aea_emissions`.

```{r step5_walk_load_aea_emissions}
# Outline for load_aea_emissions()
# 1) Read and validate inputs
# 2) Build intermediate objects
# 3) Solve / iterate / transform as needed
# 4) Return cleaned outputs
...
This chunk defines `attach_production_emissions()` for step5.

```{r step5_functions_attach_production_emissions}
...
This chunk runs step5 and produces the step outputs.

```{r step5_run_setup}
validate_tutorial_config(emissions_config)
iot_emis <- download_or_load_iot(emissions_config)
if (isTRUE(emissions_config$enforce_co2_consistency)) {
  iot_emis <- align_iot_to_aea(iot_emis, emissions_config)
}
...
This chunk attaches emissions and assembles the Step 5 comparison table.
...
This chunk plots results for step5.
...
This chunk assembles the final comparison snapshot table.
