...
- Wages `W_t = omega * Y_t`
- Profits `Pi_t = Y_t - W_t`
- Distributed profits `Pi^d_t = lambda_pi * Pi_t` (baseline in Step 1: `lambda_pi = 1`)
- Taxes `T_t = theta * (W_t + Pi^d_t)`
- Household Disposable Income `YD_t = W_t + Pi^d_t - T_t`
...
- Fixed point of GDP in this baseline (`lambda_pi = 1`): `Y* = G / theta`
...
|---|---:|---:|---:|---:|
| Wages `W` | `+W` | `0` | `-W` | `0` |
| Distributed profits `Pi^d` | `+Pi^d` | `0` | `-Pi^d` | `0` |
| Taxes `T` | `-T` | `+T` | `0` | `0` |
| Consumption `C` | `-C` | `0` | `+C` | `0` |
| Government demand `G` | `0` | `-G` | `+G` | `0` |

From column sums:
- Households: `Delta H = YD - C = W + Pi^d - T - C`
- Government: `Delta B = G - T`
- Production: `Y = C + G = W + Pi^d`

Steady-state derivation (SIM):
- This derivation uses the Step 1 baseline closure `lambda_pi = 1`, so household pre-tax income is `W + Pi^d = Y`.
...
simulate_sim_lag <- function(T = 20, G = 100, theta = 0.2, alpha1 = 0.6, alpha2 = 0.2,
                             omega = 0.75, lambda_pi = 1, Y0 = 80, H0 = 300) {
  out <- data.frame(t = seq_len(T), Y = NA_real_, W = NA_real_, Pi = NA_real_, Pi_d = NA_real_,
                    T_tax = NA_real_, YD = NA_real_, C = NA_real_, H = NA_real_)
  W0 <- omega * Y0
  Pi0 <- Y0 - W0
  Pi_d0 <- lambda_pi * Pi0
  T0 <- theta * (W0 + Pi_d0)
  ...
  for (tt in seq_len(T)) {
    ...
    W_t <- omega * Y_t
    Pi_t <- Y_t - W_t
    Pi_d_t <- lambda_pi * Pi_t
    T_t <- theta * (W_t + Pi_d_t)
    YD_t <- W_t + Pi_d_t - T_t
    H_t <- H_prev + YD_t - C_t
    out[tt, c("Y", "W", "Pi", "Pi_d", "T_tax", "YD", "C", "H")] <-
      c(Y_t, W_t, Pi_t, Pi_d_t, T_t, YD_t, C_t, H_t)
    YD_prev <- YD_t
    H_prev <- H_t
  }
  out
...
omega <- 0.75 # wage share in output
lambda_pi <- 1 # share of profits distributed to households
...
lag_low <- simulate_sim_lag(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2,
                            omega = omega, lambda_pi = lambda_pi, Y0 = Y0_low, H0 = H0_low)
...
Current disposable income `YD_t` depends on current output and distribution assumptions.
...
simulate_sim_current_fp <- function(T = 20, G = 100, theta = 0.2, alpha1 = 0.6, alpha2 = 0.2,
  ...
  for (tt in seq_len(T)) {
    ...
    repeat {
      n_iter <- n_iter + 1L
      W_guess <- omega * Y_guess
      Pi_guess <- Y_guess - W_guess
      Pi_d_guess <- lambda_pi * Pi_guess
      T_guess <- theta * (W_guess + Pi_d_guess)
      YD_guess <- W_guess + Pi_d_guess - T_guess
      C_guess <- alpha1 * YD_guess + alpha2 * H_prev
      Y_new <- G + C_guess
...
lag_high_play <- simulate_sim_lag(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2,
                              omega = omega, lambda_pi = lambda_pi, Y0 = Y0_high_play, H0 = H0_high_play)
cur_low <- simulate_sim_current_fp(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2, omega = omega, lambda_pi = lambda_pi, Y0 = Y0_low, H0 = H0_low)
cur_high <- simulate_sim_current_fp(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2, omega = omega, lambda_pi = lambda_pi, Y0 = Y0_high_play, H0 = H0_high_play)
...
lag_opt <- simulate_sim_lag(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2_play, omega = omega, lambda_pi = lambda_pi, Y0 = Y0_low, H0 = H0_low)
cur_opt <- simulate_sim_current_fp(T = T_h, G = G, theta = theta, alpha1 = alpha1, alpha2 = alpha2_play, omega = omega, lambda_pi = lambda_pi, Y0 = Y0_low, H0 = H0_low)
...
transition_speed_emis_play <- 0.003  # @exercise[id=step6_transition;kind=core;question_expr=0.0;prompt="Compare baseline and transition CO2 paths";hint="Use 0 for baseline and positive speed for transition"]
...
intensity_decline_play <- 0.01  # @exercise[id=step6_intensity;kind=optional;question_expr=0.0;prompt="Apply annual intensity decline and compare CO2 path";hint="This is exogenous decarbonization of direct intensity"]
