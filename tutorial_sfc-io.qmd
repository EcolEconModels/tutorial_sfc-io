...
Y0_high_play <- 1.3 * Y_star  # @exercise[id=step1_high_start;kind=core;question_expr=1.1 * Y_star;prompt="Start GDP above fixed point and compare convergence";hint="Try 1.1, 1.3, then 1.6 times Y*"]
...
Define compact IOT loader (and keep exogenous-transition function for later advanced step).

This block builds the core IO objects and the advanced exogenous-transition solver wrapper.
Pseudo-code (loader): identify sectors, compute Z/A/L, and extract C/G/I/EX vectors from IO uses.
Pseudo-code (calibration): compute import leakage m_i, value-added coefficients, and beta demand shares.
Pseudo-code (exogenous transition): each year call closure solver, update X/F, then store GDP and sector rates.
This keeps static IO calibration separate from dynamic closure experiments.
...
```{r step2_functions_sim_iot, echo: false}
...
Construct the SIM+IO calibration object.
This object feeds all Step 3 simulations.
...
Change consumption out of wealth (`c_v`) under endogenous transition.
Rerun with fixed transition speed and altered `c_v`.
Inspect wealth dynamics under the transition setting.
```{r step3_play_optional_cv}
c_v_play <- 0.06  # @exercise[id=step3_cv;kind=optional;question_expr=0.01;prompt="Change c_v and inspect wealth and GDP trajectories";hint="High c_v can sustain higher demand but deplete wealth"]
...
Calibrate RoW-lite using the core IO and wealth data.
Run the baseline path with zero transition speed.
This provides the trade-balance reference.
...
```{r step6_functions_emissions, echo: false}
