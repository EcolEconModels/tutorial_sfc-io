...
Step 4 opens the Step 3 economy by separating domestic final demand, imports, and exports.
The closure moves from a closed identity (`Y = C + G + I`) to an open one (`Y = C + G + I + EX - IM`) while keeping SIM stock-flow accounting.

Closed-to-open decomposition:
- Domestic final demand by sector: `FD_dom,i,t = C_i,t + G_i,t + I_i + EX_i,t`
- Import leakage by sector: `IM_i,t = m_i * FD_dom,i,t`
- Net domestic demand for domestic production: `FD_net,i,t = FD_dom,i,t - IM_i,t`
- Production mapping: `x_t = L * FD_net,t`
- Trade aggregates: `EX_t = sum_i EX_i,t`, `IM_t = sum_i IM_i,t`, `TB_t = EX_t - IM_t`

Macro block remains SIM-consistent:
...
- `m_i`: sector-specific import propensity applied to domestic final-demand uses.
- `EX_i,t`: exogenous export demand for domestic sector `i`.
- `FD_net`: portion of final demand satisfied by domestic production technology `A`.
- `TB_t`: trade balance; in this minimal setup it is the period change in net foreign asset position.
- `Delta_NFA_t = TB_t`: external stock-flow bridge.
