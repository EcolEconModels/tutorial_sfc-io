...
This step reuses helper calibration/loading and defines RoW-lite dynamics locally.
...
- `simulate_sim_iot_row_lite()` is defined in chunk `step4_functions_row_lite`.

Where key variables are set in this step:
- `core_config` is created/printed in `step4_run_config`.
- RoW calibration object `calib4` and baseline run `step4_baseline` are built in `step4_run_baseline`.
- Calibration diagnostics are printed in `step4_calibration_snapshot`.
- Core play variable `import_leakage_scale_play` is set in `step4_play_core_imports`.
- Optional play variable `ex_growth_play` is set in `step4_play_optional_exports`.

## Step 4: RoW-lite

### Objective

Open the calibrated SIM-IOT baseline by adding exports, import leakage, and trade-balance accounting in a minimal RoW-lite closure.
...
- In this RoW-lite implementation, imports are modeled as proportional leakage from domestic final-demand uses.
...
|---|---:|---:|
| Net foreign asset stock `NFA` | `+NFA` | `-NFA` |
...
- In a full sector-expanded BSM, domestic `NFA` would still be aggregate while domestic firm/household/government financial positions could be disaggregated.
- The compact BSM shown here keeps only the external stock needed to close the open-economy SFC identities.
...
Detailed loop logic for `simulate_sim_iot_row_lite()`:
...
plot(step4_baseline$year, step4_baseline$GDP, type = "l", lwd = 2, col = "steelblue",
     xlab = "year", ylab = "GDP", main = "Step 4 Core: GDP")
...
plot(step4_baseline$year, step4_baseline$TB, type = "l", lwd = 2, col = "steelblue",
     xlab = "year", ylab = "TB = EX - IM", main = "Step 4 Core: trade balance")
...
- RoW-lite is not a substitute for MRIO when foreign technology and embodied emissions matter quantitatively.
