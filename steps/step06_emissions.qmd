...
Step 6 converts sector output paths into direct production emissions using AEA-aligned intensities.
The macro-production simulation is unchanged; only the accounting layer for emissions is added.

Production emissions accounting:
- Sector intensity (kg per monetary output): `s_i = CO2_i / x_i`
- Period production emissions: `CO2_t = sum_i(s_i * x_i,t)`
- Reporting conversion: `CO2_Mt,t = CO2_t / 1e9`

Data alignment logic:
- IO sectors are normalized/matched to AEA NACE categories.
- Intensities are estimated from base-year AEA emissions and base-year IO output.
- Simulated `x_i,t` paths (from RoW-lite loop) are multiplied by fixed `s_i` unless sensitivity modifies intensities.

Economic meaning of symbols:
- `x_i,t`: domestic production in sector `i` from SIM-IOT-RoW dynamics.
- `s_i`: direct production emission intensity for sector `i`.
- `CO2_t`: production-based (territorial production) emissions, not consumption footprint.
- `intensity_decline`: optional exogenous factor for technical decarbonization sensitivity.

Boundary note (equations/text only):
- Consumption-based emissions would add imported emission content using foreign technologies (MRIO-style).
- This tutorial implements production-based emissions only, to keep boundaries explicit and minimal.
...
Step 6 is the first time emissions helpers are used.  
We keep full implementations in `R/helpers/emissions_load.R` and show key excerpts so participants can follow the alignment logic.

NACE-code normalization and alignment:
...
align_iot_to_aea <- function(iot, cfg) {
  ...
  keep <- normalize_nace_code(iot$sector_codes) %in% aea_norm
  # keep only sectors that exist in both IO and AEA classifications
}
```

Intensity construction and aggregation:

```r
intensity <- (em_ths_t * 1e6) / pmax(as.numeric(x0), 1e-9)
co2_kg <- as.numeric(sim_result$x %*% intensity_vector)
CO2_Mt <- co2_kg / 1e9
```
